...
export const site = {
  ...
  /**
   * Заготовка сообщения в WhatsApp. Обязана быть законченной фразой,
   * которую можно отправить, ничего не печатая, — иначе человек упирается
   * в недописанное предложение и закрывает приложение.
   *
   * Так и было в первую неделю рекламы: заготовка кончалась на
   * «Холодильник: [модель]. Проблема: », то есть требовала заменить
   * «[модель]» и дописать симптом. За четыре дня из рекламы пришло 8
   * нажатий кнопки WhatsApp (GA4, канал Paid Search) и ни одного
   * сообщения мастеру. Проверка показала, что сам канал исправен —
   * сообщения, отправленные вручную, доходят.
   */
  whatsappText: 'Здравствуйте! Пишу с сайта, нужен ремонт холодильника.',
