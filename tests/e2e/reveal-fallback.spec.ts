...
test.describe('фолбэк анимации появления в браузере без animation-timeline: view()', () => {
  test('с работающим JS: контент скрывается и затем становится видимым', async ({ page }) => {
    ...
    // Пробник — реальный заголовок секции «Что случилось» (Breakages), а
    // не тестовая заглушка: обёртка SectionHeading.astro несёт ровно класс
    // "reveal" (не "reveal-soft"/"reveal-stagger" — те не совпадают с
    // точным CSS-классом .reveal), секция стоит третьей на странице, то
    // есть заведомо вне первого экрана на обоих проектах (мобильном и
    // 1440×900), и под ней всегда есть другой контент (HowItWorks и далее),
    // так что для неё не возникает вырожденного случая «упёрлись в конец
    // документа» — в отличие от прежнего пробника, для которого ниже
    // ничего не было. IntersectionObserver ставит is-visible только когда
    // элемент реально попадает во вьюпорт — это и есть проверяемое
    // поведение фолбэка, а не удобство теста, поэтому докручиваем до
    // заголовка, а не полагаемся на то, что он виден без скролла.
    //
    // Обычный scrollIntoViewIfNeeded() здесь недостаточен: он ставит
    // элемент вплотную к краю вьюпорта. У IntersectionObserver задан
    // rootMargin '0px 0px -10% 0px' (полифилл нарочно игнорирует последние
    // 10% высоты вьюпорта снизу, чтобы не засчитывать элемент, едва
    // показавшийся из-за края), поэтому элемент на самой кромке рискует не
    // пересечь зону срабатывания. Прокручиваем так, чтобы заголовок встал
    ...
    const probe = page.locator('[data-section="breakages"] .reveal').first();
